###Generation of a collapsible Tree Visualization####
...
.collapsibleTree -> function(){
  ...
  nodes = data.frame(as.vector(unique(unlist(network))),
                     ...
                     c(rep("C",EAD$NUMB_C),rep("CN",EAD$NUMB_CN),rep("FR",EAD$NUMB_FR),rep("CM",EAD$NUMB_CM),
                       rep("PV",EAD$NUMB_PV),rep("RC",EAD$NUMB_RC)))
  substr(as.vector(unique(unlist(network))),1,2)
