...
Der Demand wird in Form eines Vektors zufällig generiert, hierzu wird die Funktion _.gen_Demand_ aus __gen_Q.R__ ausgeführt.In dieser Funktion werden zunächst mittels _runif()_ zufällige Werte mit einer Standartabweichung zwischen Q_VAR_MIN/MAX erzeugt. Anschließend werden die Werte auf TQ normiert und gerundet und absteigend in den Vektor sortiert.
...
Ausnahme. Hier wird die Matrix durch die Funktion _.gen_RES_CONS_PAT()_ erstellt. Dies funktionietrt im Prinzip wie die demand generation.
```{R, echo=FALSE}
